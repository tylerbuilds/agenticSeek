"""
Example Tool: Flight Search
Source: /mnt/data/projects/agfentSeek/sources/tools/flightSearch.py

This example demonstrates how to create a custom tool for AgenticSeek that:
1. Inherits from the Tools base class
2. Implements all required abstract methods
3. Makes external API calls (SerpApi)
4. Handles errors gracefully
5. Provides feedback to the LLM
"""
...
class FlightSearch(Tools):
    def __init__(self, api_key: str = None):
        ...
        Args:
            api_key: Optional API key for SerpApi. If not provided, will try to load from env.
        """
        super().__init__()
    ...
    def execute(self, blocks: str, safety: bool = True) -> str:
        """
        Execute flight search for the provided flight number.
        
        Args:
            blocks: List of blocks containing flight numbers to search
            safety: Whether to require human intervention (not used in this tool)
            
        Returns:
            str: Formatted flight information or error message
        """
    ...
    def interpreter_feedback(self, output: str) -> str:
        """
        Generate feedback for the LLM based on execution results.
